...
impl PixbufLoader {
    ...
    pub fn new_with_type(image_type: &str) -> Result<PixbufLoader, Error> {
        ...
        let tmp = unsafe { ffi::gdk_pixbuf_loader_new_with_type(image_type.lend_to_glib().0, &mut error) };
    ...
    pub fn new_with_mime_type(mime_type: &str) -> Result<PixbufLoader, Error> {
        let mut error = ptr::null_mut();
        let tmp = unsafe { ffi::gdk_pixbuf_loader_new_with_mime_type(mime_type.lend_to_glib().0, &mut error) };
